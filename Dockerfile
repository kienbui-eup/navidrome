...
RUN --mount=type=bind,source=. \
    ...
    go build -tags=${BUILD_TAGS} -ldflags="-w -s \
        ...
        -X github.com/vi2play/vi2play/consts.gitSha=${GIT_SHA} \
        -X github.com/vi2play/vi2play/consts.gitTag=${GIT_TAG}" \
        -o /out/navidrome .
...
LABEL org.opencontainers.image.source="https://github.com/vi2play/vi2play"

# Install runtime dependencies
# - libwebp + symlinks: enables native WebP encoding via purego/dlopen
RUN apk add -U --no-cache ffmpeg mpv sqlite libwebp libwebpdemux libwebpmux python3 nodejs quickjs curl ca-certificates && \
